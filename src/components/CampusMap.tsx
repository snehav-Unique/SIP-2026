  ...
  return (
    <div className="absolute bottom-4 right-4 z-[400]">
      <button
        ...
        className="flex h-11 w-11 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-700 shadow-lg transition-colors hover:text-primary disabled:opacity-50"
      >
...
function getGeoStatusFromError(
  code: number,
): Exclude<GeoState["status"], "idle" | "requesting" | "active"> {
...
export default function CampusMap({
  ...
  const beginGeoWatch = () => {
    ...
    setGeoState((current) =>
      current.status === "active"
        ? current
        : { status: "requesting", position: null, message: null },
    );
  ...
  const routePath = routeState.status === "ready" ? routeState.path : null;
    ...
    <div className="space-y-4">
      {geoState.status !== "active" ? (
        <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-stone-900">Live location</p>
              ...
              {"message" in geoState && geoState.message && (
      ...
      {geoPosition && selectedLocation ? (
        <div className="rounded-2xl border border-primary/15 bg-primary/5 p-4 shadow-sm">
          <p className="text-sm font-semibold text-stone-900">Route preview active</p>
        ...
        <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-semibold text-stone-900">Destination selected</p>
          <p className="mt-1 text-xs text-stone-500">
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
          ...
          <div className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center gap-1">
            {search && (
              <button
                ...
                className="p-0.5 text-stone-400 transition hover:text-stone-600"
              >
                <X size={15} />
              </button>
            )}
            ...
            >
              <ChevronDown
                ...
                className={`transition-transform duration-200 ${
                  browseAll && dropdownOpen ? "rotate-180" : ""
                }`}
              />
            </button>
          </div>
        ...
        {debouncedSearch && !pinnedLocation && suggestions.length === 0 ? (
          ...
          <p className="mt-2 text-xs text-stone-500">
            Showing <span className="font-semibold">{pinnedLocation.name}</span>
          </p>
        ...
        <div className="h-[60vh] min-h-[420px] w-full sm:h-[65vh]">
          ...
          >
            ...
            <RouteController
              ...
              userPosition={
                geoPosition ? { lat: geoPosition.lat, lng: geoPosition.lng } : null
              }
              routePath={routePath}
            />
            ...
            <LocateControl onLocate={beginGeoWatch} locating={geoState.status === "requesting"} />
            {visibleGroups.map((group) => {
              ...
              return (
                <Marker
                  ...
                  icon={createCustomIcon(isActive || isFocusedVenue, group.announcements.length > 0)}
                  eventHandlers={{
                    click: () => {
                      ...
                      <div>
                        ...
                        {group.announcements.length > 0 && (
                          <p className="mt-2 text-xs font-medium text-primary">
                            {group.announcements.length} active venue event
            ...
            {venueGroups.map((group) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <p className="font-semibold text-stone-900">{group.location.name}</p>
